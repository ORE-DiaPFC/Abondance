...
### FISH NUMBER STAYING IN LN1 (n_11)
  for (t in 1:8) { ## 1984 -> 1991 (p_LN1_1 exchangeable before 1992)
    ...
    n_11[t,g] ~ dbin(p_11_1[t],n[t,g])#I(lim_11[t,g],)
    constraint_11[t, g] ~ dconstraint(n_11[t, g] >= lim_11[t, g])
    } ## End of loop over years
...
for (t in 7:Y) { # from 1990 to now on
  e_21_tot[t] <- sum(e_21[t,1:4]) ; e_22_tot[t] <- sum(e_22[t,1:4])
  } ## End of loop over years
...
for (t in 1:6) {
  ...
  no_ech_1SW[t] <- n_1SW[t] - ech_1SW_tot[t] # Individuals 1SW not sampled
  ...
  ### Individuals MSW
  ech_MSW[t,1:5] ~ dmulti(p_MSW[t,1:5], ech_MSW_wild[t]) # Individuals MSW sampled
  ...
  no_ech_MSW_tot[t] <- n_MSW[t]- ech_MSW_tot[t] # Individuals MSW not sampled
  no_ech_MSW_wild[t] ~ dbin(p_MSW_wild[t], no_ech_MSW_tot[t]) # Individuals MSW wild not sampled
  ...
  p_MSW[t,1:5] ~ ddirich(a_MSW[1:5]) # annual probability to sample a MSW individual. 
  } # end of loop over years
...
for (t in 7:Y) {
  ...
  n_1.1SW[t] <- ech_1.1SW[t] + no_ech_1.1SW[t] # Total number of individuals 1SW
  n_2.1SW[t] <- n_1SW[t] - n_1.1SW[t] # Individuals 2R/1SW
  
  ### Individuals MSW
  ech_MSW[t,1:5] ~ dmulti(p_MSW[t,1:5], ech_MSW_tot[t]) # Individuals MSW sampled
...
a_1.1SW ~ T(dexp(0.25) ,0.02,)  # shape parameter of the beta distribution for p_1.1SW
a_2.1SW ~ T(dexp(0.25) ,0.02,)  # shape parameter of the beta distribution for p_1.1SW
a_MSW[1] ~ T(dexp(0.25) ,0.02,) # 1R/2SW
a_MSW[2] ~ T(dexp(0.25) ,0.02,) # 2R/2SW
a_MSW[3] ~ T(dexp(0.25) ,0.02,) # 1R/3SW
a_MSW[4] ~ T(dexp(0.25) ,0.02,) # 2R/3SW
a_MSW[5] ~ T(dexp(0.25) ,0.02,) # Second spawning
