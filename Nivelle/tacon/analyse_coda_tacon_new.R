...
LN_new <- LN_tmp[31]+(17*(1:10)) # incremente par 17 chaque année jusqu'en 2024
LN <-c(LN_tmp,LN_new) # ajout à la fin du vecteur
HN_tmp <- c(rep(NA,19), 453, NA, 467, 474, 484, 494, 516, 533, 550, 567, 584, 601)  # Haute-Nivelle
HN_new <- HN_tmp[31]+(17*(1:10)) # incremente par 17 chaque année jusqu'en 2023
HN_new[10] <- HN_new[10]-1 # une station de moins sur LN
HN <-c(HN_tmp,HN_new) # ajout à la fin du vecteur
VHN <- rep(NA,41) # introduction de la Nivelle espagnole en 2024
VHN[41] <- 774 # index du premier site de la Nivelle espagnole échantillonné en 2024
LUR_tmp <-  c(rep(NA,19), 455, NA, 504, 456, 506, 509, 526, 543, 560, 577, 594, 611) # Lurgorrieta
LUR_new <- LUR_tmp[31]+(17*(1:10)) # incremente par 17 chaque année
LUR_new[10] <- LUR_new[10]-2 # ajustement pour modification du nombre de station en 2024
LUR <-c(LUR_tmp,LUR_new) # ajout à la fin du vecteur
# Instructions à suivre à mettre à partir de 2026
# LN_new <- LN[41]+(15*(1:(Y_last-41))) # incremente par 15 chaque année à partir de 2025
# LN <-c(LN,LN_new) # ajout à la fin du vecteur
#HN_new <- HN[41]+(15*(1:(Y_last-41))) # incremente par 15 chaque année à partir de 2025
#HN <-c(HN,HN_new) # ajout à la fin du vecteur
#VHN_new <- VHN[41]+(15*(1:(Y_last-41))) # incremente par 15 chaque année à partir de 2025
#VHN <-c(VHN,VHN_new) # ajout à la fin du vecteur
#LUR_new <- LUR[41]+(15*(1:(Y_last-41))) # incremente par 15 chaque année à partir de 2025
#LUR <-c(LUR,LUR_new) # ajout à la fin du vecteur
START_i_CPUE <-cbind(LN1,LN2,LN,HN,LUR,VHN)
...
HN <-c(HN_tmp,HN_new) # ajout à la fin du vecteur
VHN <- rep(NA,41) # introduction de la Nivelle espagnole en 2024
VHN[41] <- 778 # index du premier site de la Nivelle espagnole échantillonné en 2024
...
#LUR_new <- LUR[41]+(15*(1:(Y_last-41))) # incremente par 15 chaque année à partir de 2025
#LUR <-c(LUR,LUR_new) # ajout à la fin du vecteur
END_i_CPUE <-cbind(LN1,LN2,LN,HN,LUR,VHN)
...
# Ancienne version utilisée jusqu'en 2024
...
# Version mise en oeuvre à compter de 2024 avec la redistribution des sites de pêche
if (last(sites$Y)<Y_last) {
Site <- (last(sites$Site)+1) : (last(sites$Site)+15)
Nat <- (last(sites$Nat)+1) : (last(sites$Nat)+15)
Comp <- rep(NA,15)
Res <- rep(NA,15)
Y <- rep(Y_last,15)
Z <- c(rep(3,3),rep(4,4),rep(7,5),rep(5,3))
S <- c(449,462,1462,777,523,1138,660,660,660,660,660,660,230,325,931)
H <- rep(1,15)
...
jVHN_nat_ns <- jVHN_res_ns <- jLAP_res_ns <- jVHN_ns <- jLAP_ns <- matrix(0,nrow=n_iter,ncol=Y_last)
...
jVHN <- jVHN_nat <- jVHN_res <- matrix(0,nrow=n_iter,ncol=Y_last)
...
setTxtProgressBar(pb,i)
   ...
   # }# end of loop over iterations
    ...
    # From 2024 to now on  
    for (y in 41:Y_last) {
        jVHN[i,y] <- jVHN_nat[i,y] <- jVHN_nat_ns[i,y]
        } ## End of loop over years
...
setTxtProgressBar(pb,i)
    for (y in 2:Y_last) {
        ...
        YOYnat[i,y] <- jLN_nat[i,y]+jHN_nat[i,y]+jVHN_nat[i,y]+jLUR_nat[i,y]
...
jVHN_q <- jVHN_nat_q <- jVHN_res_q <- matrix(NA,nrow=Y_last,ncol=9) 
...
## Summary statistics per zone and type of reproduction ##
  ...
  jVHN_nat_q[y,1] <- mean(jVHN_nat[,y])
  ...
  jVHN_nat_q[y,3:9] <- quantile(jVHN_nat[,y],probs,names=FALSE)
...
write.table(jVHN_nat_q,file="results/jVHN_nat.txt", row.names=F, col.names=cnames, sep = "\t") 
...
Num1_VHN <- Num1_nat_VHN <- Num1_res_VHN <- Num1_LAP <- Num1_res_LAP <- matrix(0,nrow=n_iter,ncol=Y_last)
...
setTxtProgressBar(pb,i)
  ...
  # From year 2024 to now on
  for(y in 41:Y_last) {
  Num1_VHN[i,y] <- Num1_nat_VHN[i,y] <- sum(dj_wth[i,START_i_CPUE[y,6]:END_i_CPUE[y,6]])
    } ## End of loop over years
  } ## End of loop over iterations
...
setTxtProgressBar(pb,i)
  ...
  #From 2024 to now on
    for(y in 41:Y_last) {
    Den1_VHN[i,y] <- sum(S_req[i,START_i_CPUE[y,6]:END_i_CPUE[y,6]])
    } ## End of loop over years
...
mu_dj_wth_VHN <- mu_djnat_wth_VHN <- mu_djres_wth_VHN <- matrix(0,nrow=n_iter,ncol=Y_last)
...
setTxtProgressBar(pb,i)
    ...
    mu_djnat_wth_VHN[i,y] <- Num1_nat_VHN[i,y]/max(Den1_VHN[i,y],1)
...
setTxtProgressBar(pb,i)
  ...
  Stot_req_new[i,7] <- data$S_tot_new[7,1] + (data$S_tot_new[7,2] *beta[i,2]) # surfaces actualisée à partir de 2024
  Stot_req[i,8] <- data$S_tot[8,2] + (data$S_tot[8,2] *beta[i,2])
    ...
    for (y in 41:Y_last) {
        mu_dj_wth_y[i,y] <- (mu_dj_wth_LN[i,y]*Stot_req[i,3] + mu_dj_wth_HN[i,y]*Stot_req[i,4] +
                         mu_dj_wth_LUR[i,y]*Stot_req[i,5] + mu_dj_wth_VHN[i,y]*Stot_req_new[i,7]) / 
                         ((I_prod[y,3]*Stot_req[i,3])+ (I_prod[y,4]*Stot_req[i,4]) + (I_prod[y,5]*Stot_req[i,5]) + 
