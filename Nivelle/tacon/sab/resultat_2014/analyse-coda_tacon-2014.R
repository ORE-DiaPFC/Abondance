...
data.dir <- paste("data/",stade,"/","data-",stade,"-",year,".txt",sep="")
data <- read.bugsdata(data.dir)

# loading coda
load(paste('results/Results_',stade,"_",year,'.RData',sep=""))
out.mat <- as.mcmc(fit) # using bugs
...
n_iter <- dim(out.mat)[1]   # 10 000 iterations * 3 chaines MCMC
...
#out <- read.openbugs(stem="M:\\Nivelle\\Juvenile-2014\\coda\\", start = 10001, end = 20000, quiet=T)
#out.mat <- as.matrix(out)
#colnames(out.mat) <- varnames(out)
...
## Nat: idem que Site;;;;;;;
sites <- read.csv("data/tacon/sites.csv",header=T,sep=";",dec=".") # /!\ This file needs to be updated every year
...
if (last(sites$Y)<Y_last) {
  ...
  write.table(sites_new,file="data/tacon/sites.csv",sep=";",row.names = FALSE)
}



# Pas de mise ? jour ici
jcomp_ns_riff <- out.mat[,(which(colnames(out.mat)=="jcomp_ns_riff[13,3]", arr.ind=T)):(which(colnames(out.mat)=="jcomp_ns_riff[26,3]", arr.ind=T))]
jcomp_ns_runs <- out.mat[,(which(colnames(out.mat)=="jcomp_ns_runs[13,3]", arr.ind=T)):(which(colnames(out.mat)=="jcomp_ns_runs[26,3]", arr.ind=T))]
jnat_ns_riff <- out.mat[,(which(colnames(out.mat)=="jnat_ns_riff[2,3]", arr.ind=T)):
    (which(colnames(out.mat)==(paste("jnat_ns_riff[",Y_last,",5]",sep="")), arr.ind=T))]
jnat_ns_runs <- out.mat[,(which(colnames(out.mat)=="jnat_ns_runs[2,3]", arr.ind=T)):
    (which(colnames(out.mat)==(paste("jnat_ns_runs[",Y_last,",5]",sep="")), arr.ind=T))]
jres_ns_riff <- out.mat[,(which(colnames(out.mat)=="jres_ns_riff[3,4]", arr.ind=T)):(which(colnames(out.mat)=="jres_ns_riff[12,8]", arr.ind=T))]
jres_ns_runs <- out.mat[,(which(colnames(out.mat)=="jres_ns_runs[3,4]", arr.ind=T)):(which(colnames(out.mat)=="jres_ns_runs[12,8]", arr.ind=T))]
n1 <- out.mat[,(which(colnames(out.mat)=="n1[1]", arr.ind=T)):(which(colnames(out.mat)=="n1[445]", arr.ind=T))]
n1_comp <- out.mat[,(which(colnames(out.mat)=="n1_comp[248]", arr.ind=T)):(which(colnames(out.mat)=="n1_comp[445]", arr.ind=T))]
n1_nat <- out.mat[,(which(colnames(out.mat)=="n1_nat[248]", arr.ind=T)):(which(colnames(out.mat)=="n1_nat[445]", arr.ind=T))]
beta <- out.mat[,"beta_dj[2]"]
dj <- out.mat[,(which(colnames(out.mat)=="dj[1]", arr.ind=T)):(which(colnames(out.mat)==(paste("dj[",I,"]",sep="")), arr.ind=T))]
dj_comp <- out.mat[,(which(colnames(out.mat)=="dj_comp[248]", arr.ind=T)):(which(colnames(out.mat)=="dj_comp[515]", arr.ind=T))]
dj_nat <- out.mat[,(which(colnames(out.mat)=="dj_nat[248]", arr.ind=T)):(which(colnames(out.mat)=="dj_nat[515]", arr.ind=T))] 
...
for (i in 1:n_iter) {
  ...
  for (y in 2:7) { 
    ...
    #jLN_samp[i,y] <- sum(out.mat[,paste("n1[",START_i[y,3],"]"):paste("n1[",END_i[y,3],"]")])     
