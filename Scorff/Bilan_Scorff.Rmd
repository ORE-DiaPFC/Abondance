...
plot(NULL,xlim=c(1,data$Y),ylim=c(0,100),bty="n",ylab="Proportion de mâles (en %)",xaxt="n",xlab="",main="Proportion de mâles par âge de mer")
axis(side=1,line=1,labels = years[-1],at=1:data$Y, las=2, cex=.5)

# N total
segments(1:data$Y,piMP_MSW[,"2.5%"], 1:data$Y,pmale_MSW[,"97.5%"], col=paste0(mycol[2]))
...
tmp <- data.frame(Years=1993:2022, MSW=piD_MSWum[,"50%"], OneSW=piD_1SWum[,"50%"])
library(knitr)
...
axis(side=1,line=1,labels = 1994:2022,at=1:data$Nyears, las=2, cex.axis=.6)
...
axis(side=1,line=1,labels = 1995:2023,at=1:data$Nyears, las=2, cex.axis=.7)
...
ratio <- smolt[1:29,"50%"]/parr[2:(nrow(parr)-1),"50%"]
plot(ratio,bty="n",ylab="Ratio Smolts 1+/Tacons 0+",xlab="Cohort",xaxt="n",ylim=c(0,1))
axis(side=1,line=0,labels = 1994:2022,at=1:29, las=2, cex=.5)


  df <- data.frame(x=1:29, y= ratio)
  lw1 <- loess(y ~ x,span = 0.75, degree = 2, data=df)
  pred <- predict(lw1)
