...
# 1/ If  Leslé was NOT STOPPED due to flow, only needs to add one line of code: p_ML[x] <- p[x,2]
######### where x is the index of the last year (=Nyear); see L. 180 for an example; add this line after L.182
# 2/ If Leslé was STOPPED due to flow, needs to update code at 2 places:
# - add one to the upper index of the loop (L.149 where j is the number of years where Leslé was stopped during the trapping season = 7 until 2014)
...
# C_ML[t]: Annual number of smolt captured at Moulin de Leslé
# Cum_ML[t]: Annual number of smolt captured at Moulin de Leslé and released unmarked downstream Moulin de Leslé
# Cm_ML[t]: Annual number of smolt marked and released downstream Moulin de Leslé
...
# Q[t,i]: Mean flow observed from 1st April to May 10 for 1. Moulin des Princes and 2. Moulin de Leslé
########################################################################
# NOTATION:
# i: trap, 1: Moulin des Princes; 2: Moulin de Leslé
# p_ML[t]: annual trap efficiency for capturing smolt at Moulin de Leslé
...
### Mean and standard deviation for years when Leslé is either stopped during several days due to flow or not installed whereas migration already begun (7 years)
...
for (t in 1:2) {  # first 2 years, Leslé is not installed
...
#Calculating R² = 1 -(E(variance of residuals (/!\ not standardized!) / E(variance of capture probabilities))) for Moulin de Leslé
# See Gelman & Pardoe 2006
...
###### Setting up probability of capture in Leslé for years when Leslé is stopped due to flow problem
for (j in 1:7) { # for years when Leslé was stopped during several days due to flow or installed after migration has begun (total of 7 years)
...
# Leslé(in specific years, probabilities are set to be smaller)  
...
###############################################
# Captures at Moulin des Princes since Leslé is installed (1997)
################################################ 
