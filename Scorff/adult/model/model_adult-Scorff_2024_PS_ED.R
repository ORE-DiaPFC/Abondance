...
###                 March 2020, March 2022, July 2025                        ###
################################################################################
...
##    1-1SW (Grisle), 
##    2-MSW (salmon)
##    3-PS (previous spawner -  short absence)
##    4-PS (previous spawner -  long absence)
## The last categories have been introduced in the model in 2025 and concerns only the returns at MDP trap. After trapping, there is only two sea-age categories, the MSW including the PS.
...
## Recapture during before or shortly after reproduction
## - Probability of capture at Moulin des Princes the first year (1994) is included in the hierarchy but is considered to be smaller. Captures begun very late for MSW and rocky fence not in place for 1SW. 
...
## - In 2003, fishing on 1SW was forbidden. Set p1F_1SW[10,u] = 0.
...
## Associated variance parameters are also dependent on sea age for death but not for fishing and recapture (after check against data)
############################################################################################
...
model {
  ...
  pi_MP94[3] <- pi_MP94[1]  # ep+mb July 2025
  pi_MP94[4] <- pi_MP94[2]  # ep+mb July 2025
  pi_MP20[1] <-1  # 2020 is NOT considered to be different for 1SW
  pi_MP20[2] ~ dbeta(2,2)  # 2020 is considered to be different for MSW (COVID)
  for (a in 1:2) {
    # Probabilities to be captured at Moulin des Princes.
    pi_MP94[a] ~ dbeta(2,2)  # first year 1994 is considered to be different
    ...
    # Probabilities to die from fishing or not depends on marked status and on sea age.
    for (u in 1:2) {
    ...
    # Probability of recapture by electrofishing during spwaning from 2020 (changed in 2025)
  ...
  ## -------------------------
  ## pi_MP94[a]: rate affecting probability to be captured at Moulin des Princes given sea age for 1994 
  ## pi_MP[t,a]: annual probability to be captured at Moulin des Princes given sea age
  ##  - PS introduced in 2025 (a=3,4)
  ##  - prob. of capture equal to that of 1SW (similar timing of return) for short absence (a=3)
  ##  - prob. of capture equal to that of MSW (similar timing of return) for long absence (a=4)
  ## pi_D_1SW[t,u]: annual probability for a marked 1SW to die from cause other than fishing given marking
  ## pi_D_MSW[t,u]: annual probability for a marked MSW to die from cause other than fishing given marking
  ## pi_oD: probability to recover a fish that die from other cause than fishing
  ## pi_F_1SW[t,u]: annual probability of a 1SW to be captured by fishing given marking
  ## pi_F_MSW[t,u]: annual probability of a MSW to be captured by fishing given marking 
  ## pi_oF[t,a]: annual probability to observe a caught fish (exploitation). From 1994 to 2002.
  ## pi_R[t,a]: annual probability to be (re)captured during or after reproduction given sea age
  ####################################################################################  
  ...
  for (t in 1:Y) { ## pi_MP: Exchangeable from 1994 to now on     
    ...
    pi_MP[t,3] <- pi_MP[t,1]    # introduced in 2025 (ep+mb)    
    pi_MP[t,4] <- pi_MP[t,2]    # introduced in 2025 (ep+mb)    
...
# to bechanged to vary with flow and effort like pi_R when more years are available
  ...
  ## DATA
  ## --------------
  ##  C_MP[t,a]: Annual number of fish captured at Moulin des Princes per sea age category (1SW/MSW/PS) (PS introduced in 2025)
  ...
  for (t in 1:Y) { 
    ...
    n_MSW[t] <- n[t,2] + n[t,3] + n[t,4]# Number of males and females MSW (include PS fish) 
    n_PS[t] <- n[t,3] + n[t,4] # Number of PS fish
    #p_1SW[t] <- n[t,1]/n_tot[t]
  }
  n_MSW[Y+1] <- n[Y+1,2] + n[Y+1,3] + n[Y+1,4] # prediction of MSW for the next year / ep+mb 9.07.2025
  n_PS[Y+1] <- n[Y+1,3] + n[Y+1,4] # Number of PS fish
  
  ## Distributing 1SW/MSW into sex classes - mb+ep-13.3.2024
  for (t in 1:Y) {
    ...
    # Modelling of the proportion of samples
    p_smp[t,1] ~ dbeta(2, 2)
    p_smp[t,2] ~ dbeta(2, 2)    
  } ## End of loop over years
  
  # Prior pour que les q[1:4] soient superieurs a 1 et que les priors sur p_male soient faiblement informatif
  ...
  lambda_tot1 ~ dgamma(shape_lambda,rate_lambda)
  lambda_tot0 ~ dgamma(shape_lambda,rate_lambda)
  Plambda0[1] ~ dbeta(s[1],s[2]) ; Plambda0[2] <- 1 - Plambda0[1]
  
  # Hyperprior for lambda_n
  # and for p2RX (probability of return after reproduction for modeling the PS fish) (ep+m+ July 2025)
  for (t in 1:(Y+1)) { # loop to Y+1 for prediction of MSW for the next year / ep+mb 9.07.2025
    lambda_tot[t] ~ dgamma(shape_lambda,rate_lambda) 
    #    Plambda[t,1:2] ~ ddirich(s[])
    Plambda[t,1] ~ dbeta(s[1],s[2]) ; Plambda[t,2] <- 1 - Plambda[t,1]
    p2R0[t] ~ dbeta(s[3],s[4]) # short absence (ep+m+ July 2025)
    p2R1[t] ~ dbeta(s[5],s[6]) # long absence (ep+m+ July 2025)
    P2R[t] <- P2R0[t] + (1-P2R0[t]) * P2R1[t] # long & short absence combined (ep+m+ July 2025)
  }
  ...
  lambda_n[Y+1,1] <- lambda_tot[Y+1] * Plambda[Y+1,1] #  Y+1 for prediction of 1SW for the next year / ep+mb 9.07.2025

# Initialisation of the year 0 escapement (little informative prior) (ep+mb July 2025)
#  x0 ~ dlnorm(5.5,1); e_tot0 <- round(x0)
  # Formulation for the prior on e_tot0 linking with lambda_tot0 and Plambda0[1]
  x0 ~ dlnorm(0,1) # adjustement factor for e_tot0 relative to the number of 1SW fish in year 0
  lambda_e0 <- lambda_tot0 * Plambda0[1] * x0 
  e_tot0 <- dpois(lambda_e0) # total escapement in year 0
  # Alternative formulation for the prior on e_tot1 linking with lambda_tot1
  x1 ~ dlnorm(0,1) # adjustement factor for e_tot1 relative to total returns in year -1
  lambda_e1 <- lambda_tot1 * x1
  e_tot1 <- dpois(lambda_e1) # total escapement in year -1
  ...
  n[Y+1,2] ~ dpois(lambda_n[Y+1,2]) # prediction of MSW for the next year / ep+mb 9.07.2025
  n[1,3] ~ dbin(p2R0[1],e_tot0) # (ep+m+ July 2025)
  n[1,4] ~ dbin(p2R1[1],e_tot1) # (ep+m+ July 2025)
  e_tot0r <- e_tot0 - n[1,3]
  n[2,4] ~ dbin(p2R1[1],e_tot0r) # (ep+m+ July 2025)
    for (t in 2:Y+1) {
      n[t,3] ~ dbin(p2R[t],e_tot[t-1]) # annual number of short absence fish from previous escapement (ep+m+ July 2025)
    }
    for (t in 3:Y+1) {
      e_totr[t-2] <- e_tot[t-2] - n[t-1,3]
      n[t,4] ~ dbin(p2R[t],e_totr[t-2]) # annual number of long absence fish from remainder escapement of the year befor the previous (ep+m+ July 2025)
    }
...
#      n_um[t,a] <- n[t,a] - C_MP[t,a] + Cum_MP[t,a]
  ...
  # 2020 COVID
  p_MP20_tot[1] <- pi_MP[27,1] # probability in 2020 to be observed is set as usual for 1SW
  p_MP20_tot[2] <- pi_MP[27,2] * pi_MP20[2] # probability in 2020 to be observed is set to be smaller /!\ MSW only!!!
  p_MP20_tot[3] <- pi_MP[27,1]
  p_MP20_tot[4] <- pi_MP[27,2]
  ## Fish number avoiding the trap or not marked (ep+m+ July 2025)
    for (t in 1:Y) {
        n_um[t,1] <- n[t,1] - C_MP[t,1] + Cum_MP[t,1]
        n_um[t,2] <- (n[t,2] - C_MP[t,2] + Cum_MP[t,2]) + (n[t,3] - C_MP[t,3]) + (n[t,4] - C_MP[t,4])# after trapping the MSWs include the PSs 
    }
  ########################################
  ...
  for (t in 1:9) { # from 2003 to now on   
    ...
    for (a in 1:2) {
      ...
      ER_1SW[t] <- C_F[t,1] / n_1SW[t]
      ER_MSW[t] <- C_F[t,2] / n_MSW[t]
  }   
  ...
  for (t in 10:26) { # from 2003 to now on
    ...
    for (a in 1:2) {
      ...
      ## Exploitation rates (ep+mb July 2025)
      ER_1SW[t] <- (Cm_F[t,1] + Cum_F[t,1]) / n_1SW[t]
      ER_MSW[t] <- (Cm_F[t,2] + Cum_F[t,2]) / n_MSW[t]
  } # end of loop over years
  ...
  for (t in 1:Y) { 
    ...
    for (a in 1:2) {
      ...
      ## -------------------------------------
      ##
  ## Dipnet fishing used up to 2022 modeled as a multinomial with two recapture events   
  for (t in 1:29) { 
    for (a in 1:2) {      
      ## Marked fish
  ...
  ## Modeled as a multinomial with two recapture events in 2020 and 2022
  ...
  ############
  # Egg deposition (ep+mb, July 2025)
  ##############  
    eggs[t,1] <- e_1SW[t] * pf_1SW * fec_1SW + e_MSW[t] * pf_MSW * fec_MSW # egg dep calculated with standard fecundity and sex ratios used to set the CLs in Brittanysw
    eggs[t,2] <- e_1SW[t] * (1-p_male[t,1]) * fec_1SW + e_MSW[t] * (1-p_male[t,2]) * fec_MSW # egg dep calculated with sex ratios estimated from the model 
    egg_dens[t,1] <- eggs[t,1]/S_ERR[t]
    egg_dens[t,2] <- eggs[t,2]/S_ERR[t]
  # Comparison with CL  
    I_egg_CL[t,1] <- step(egg_dens[t,1] - CL)  
    I_egg_CL[t,2] <- step(egg_dens[t,1] - CL)  
  }  # end of loop over years 
