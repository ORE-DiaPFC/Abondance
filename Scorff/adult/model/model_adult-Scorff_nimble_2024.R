...
model <- nimbleCode({
  ...
  for (a in 1:2) {
    ...
    logit_int_MP[a] ~ dnorm(0,1.5)#~ dunif(-10,10) # mb+ep 20.3.2025   #intercept 
    logit_flow_MP[a] ~ dnorm(0,1.5)#~ dunif(-10,10)# mb+ep 20.3.2025 #slope for flow data (1SW at MP in 15 june - 15 august, MSW in April-June)
    # Probabilities to be captured during reproduction or after
    logit_int_R[a] ~ dnorm(0,1.5)#~ dunif(-10,10)# mb+ep 20.3.2025 # intercept 
    logit_effort_R[a] ~ dnorm(0,1.5)#~ dunif(-10,10)# mb+ep 20.3.2025 # slope for recapture effort
    logit_flow_R[a] ~ dnorm(0,1.5)#~ dunif(-10,10)# mb+ep 20.3.2025 # slope for flow in December 
    ...
    # Probility of recapture by electrofishinh during spwaning from 2020 (changed in 2025)
    sigmapi_R_pulsium ~ dgamma(2.5, 2.5) #dunif(0,20) # mb+ep 20.3.2025 # standard deviation of probabilty of recapture during reproduction
  ...
  sigmapi_MP ~ dgamma(2.5, 2.5) #dunif(0,20) # mb+ep 20.3.2025 # standard deviation of probabilty of capture at Moulin des Princes
  ...
  sigmapi_R ~ dgamma(2.5, 2.5) #dunif(0,20) # mb+ep 20.3.2025 # standard deviation of probabilty of recapture during reproduction
  ...
  sigmapi_F ~ dgamma(2.5, 2.5) #dunif(0,20) # mb+ep 20.3.2025 # std dev of fishing independant of sea and marked status
  ...
  precmatF[1:2,1:2] <- inverse(covmatF[1:2,1:2])  # precision matrix
  ...
  precmatD_1SW[1:2,1:2] <- inverse(covmatD_1SW[1:2,1:2])  # precision matrix
  ...
  precmatD_MSW[1:2,1:2] <- inverse(covmatD_MSW[1:2,1:2])  # precision matrix
  ...
  for (t in 1:Y) { ## pi_MP: Exchangeable from 1994 to now on     
    for (a in 1:2) {
      ...
      stlogQ[t,a] <- (logQ[t,a] - mean(logQ[1:Y,a]))/sd(logQ[1:Y,a]) # standardized covariate
...
# to bechanged to vary with flow and effort like pi_R when more years are available
  ...
  for (t in 1:9) { # from 2003 to now on   
    censored[t,1] ~ dinterval(Cm_Fb[t,1], Cm_F[t,1])
    Cm_Fb[t,1] ~ dbin(piF_1SW[t,1],Cm_MP[t,1]) #C(Cm_F[t,1],) #1SW
    censored[t,2] ~ dinterval(Cm_Fb[t,2], Cm_F[t,2])
    Cm_Fb[t,2] ~ dbin(piF_MSW[t,1],Cm_MP[t,2])  #C(Cm_F[t,2],) #MSW 
    
    # Unmarked fish
    censored[t,3] ~ dinterval(Cum_Fb[t,1], Cum_F[t,1])
    Cum_Fb[t,1] ~ dbin(piF_1SW[t,2],n_um[t,1]) #C(Cum_F[t,1],) #1SW 
    censored[t,4] ~ dinterval(Cum_Fb[t,1], Cum_F[t,1])
  ...
  ##############  
  for (t in 1:Y){
    e_tot[t] <- sum(e_m[t,1:2]) + sum(e_um[t,1:2]) # total escapement
