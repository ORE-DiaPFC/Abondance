...
model <- nimbleCode({
  ...
  for (a in 1:2) {
    ...
    sigmapi_D[a] ~ dgamma(2.5, 2.5) #dunif(0,20)
  ...
  sigmapi_MP ~ dgamma(2.5, 2.5) #dunif(0,20) # standard deviation of probabilty of capture at Moulin des Princes
  ...
  sigmapi_R ~ dgamma(2.5, 2.5) #dunif(0,20) # standard deviation of probabilty of recapture during reproduction
  ...
  sigmapi_F ~ dgamma(2.5, 2.5) #dunif(0,20) # std dev of fishing independant of sea and marked status
  ...
  for (t in 1:29) { ## pi_R: Exchangeable from 1994 to now on
