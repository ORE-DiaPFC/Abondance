...
dataToNimble <- list(
                     ...
                     , Cm_R=data$Cm_R
                     , Cum_R=data$Cum_R
                     , Cm_R_pulsium=data$Cm_R_pulsium
                     , Cum_R_pulsium=data$Cum_R_pulsium
                     ...
                     , Cm_F=data$Cm_F
                     , Cum_F=data$Cum_F
                     #, censored=array(1,dim=c(Y,4))
                     , Cm_Fb=array(,dim=c(Y,2))
                     , Cum_Fb=array(,dim=c(Y,2))
)

constants <- list(Y=data$Y
                  ,Q=data$Q
                  ,Q_dec=data$Q_dec
                  ,censored=array(1,dim=c(Y,4))
                  )
...
#------------------------MODEL----------------------------------##
source(paste0("model/model_",stade,"-",site,"_nimble_2024.R")) # path of the model
...
## BACKUP
save(fit,file=paste('results/Results_',stade,"_",year,'_nimble.RData',sep=""))
