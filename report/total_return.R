...
total=NULL
 for (site in sites){
   ...
   quant <- array(, dim=c(length(years), 3)); colnames(quant) <- c("2.5%","50%","97.5%"); rownames(quant) <- years
    ...
    write.csv(table, file=paste('report/Total_return_byage_',site,'.csv',sep=""),row.names = FALSE)
    write.csv(returns[[paste0(site)]], file=paste('report/Total_return_',site,'.csv',sep=""),row.names = FALSE)
