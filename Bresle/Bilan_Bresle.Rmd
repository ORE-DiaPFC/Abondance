...
colnames(table) <- c( "Year","Parr 0+","Smolts","1SW (tot returns)",	"MSW (tot returns)")
...
#table[2:nrow(table),3] <- round(n_smolt,0) # 1995 to now
    ...
    table[1:6,3] <- n_smolt[1:6] # 1982 to 1987
    table[11:19,3] <- n_smolt[7:15] # 1992 to 2000
    table[21:nrow(table),3] <- n_smolt[16:length(n_smolt)] # 2002 to now
...
library(knitr)
kable(table, row.names = FALSE, caption = "Tableau bilan pour le Scorff. Seules les valeurs médianes sont reportées")
...
plot(NULL,xlim=c(1,data$Y),ylim=c(0,500),bty="n",ylab="Total number of fish",xaxt="n",xlab="")
...
plot(NULL,xlim=c(1,data$Y),ylim=c(0,500),bty="n",ylab="Total number of returns",xaxt="n",xlab="")
axis(side=1,line=1,labels = years,at=1:data$Y, las=2, cex=.5)
mycol=c("#787878", "#1E90FF", "#a1dab4", "#FF6A6A")
...
plot(NULL,xlim=c(1,data$Y),ylim=c(0,50),bty="n",ylab="% captured at Beauchamps",xaxt="n",xlab="")#,main="% MSW to be captured by fishing")
axis(side=1,line=1,labels = years,at=1:data$Y, las=2, cex=.5)
gap=0.1
# N total
...
### Total number of returns
#png("report/total_return.png",width = 780, height = 480)
plot(NULL,xlim=c(1,data$Y),ylim=c(0,100),bty="n",ylab="% captured at EU",xaxt="n",xlab="")#,main="% MSW to be captured by fishing")
axis(side=1,line=1,labels = years,at=1:data$Y, las=2, cex=.5)
gap=0.1
# N total
segments(p+gap,piMP_MSW[,"2.5%"], p+gap,piMP_MSW[,"97.5%"], col=paste0(mycol[4]))
...
### Total number of returns
plot(NULL,xlim=c(1,data$Nyears),ylim=c(0,10000),bty="n",ylab="Total number of fish",xaxt="n",xlab="")
